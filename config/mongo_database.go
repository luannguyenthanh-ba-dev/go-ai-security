...
// No need to use pointer MongoDBConfig here because we won't modify it
func NewMongoDatabase(mongoConfig MongoDBConfig) (Database, error) {
	...
	// No need for timeout context here since Connect() doesn't block
...
func (d *MongoDatabase) GetMongoClient() *mongo.Client {
	return d.Client
}

func (d *MongoDatabase) GetMongoDatabase() *mongo.Database {
	return d.Database
}
